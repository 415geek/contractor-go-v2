...
**正式站（www / 根域）只使用名为 `contractorgo-web` 的 Vercel 项目**（导入本仓库时 **Root Directory 留空**）。  
不要用「仅将 Root 设为 `mobile`」的其它 Vercel 项目绑生产域名；变量与构建上下文和 `contractorgo-web` **不是同一套**。

---

## 一、用户端 Web App：`contractorgo-web`（www.contractorgo.io）
...
## 四、常见问题：Clerk「publishableKey is invalid」与消息「Invalid or missing token」

### A. `The publishableKey passed to Clerk is invalid`（或 key=pk_test_xxxxxxxx…）

说明 **构建时** 写入静态包的 `EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY` 不是 Clerk 控制台里的真实公钥，常见原因：

1. **Vercel 里填了文档占位符**（如 `pk_test_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx`）— 必须改成 Dashboard → **API Keys** 里整串以 `pk_test_` 或 `pk_live_` 开头的值。  
2. **改错 Vercel 项目**：`www.contractorgo.io` 必须绑在 **`contractorgo-web`**（仓库根目录、`Root Directory` 留空、根目录 `vercel.json`）。若误在「Root = `mobile`」的其它项目里配变量或部署，**不会**作用到正式站；请统一到 **`contractorgo-web` → Settings → Environment Variables** 修改并 **Redeploy**。  
3. **生产环境**建议在 Clerk 使用 **`pk_live_`**，与后端的 **`sk_live_`**（Supabase Secret 里的 `CLERK_SECRET_KEY`）同属一个 Clerk 应用。

保存变量后务必 **Redeploy**，否则浏览器仍是旧 JS。

### B. 消息页报错 `Invalid or missing token`（Edge 返回 401）

前端已登录但 Edge 不认 JWT 时会出现。请按顺序检查：

1. **Supabase** → Project Settings → **Edge Functions** → **Secrets**：至少配置 **`CLERK_SECRET_KEY`**（`sk_live_...`）或 **`CLERK_JWT_KEY`**（PEM），与前端 **`pk_live_` 同属一个 Clerk 应用**。生产环境 Session 常为 **JWE**，不设此项则 `get-user` 无法解析用户 → 401。  
2. 可选：配置 **`CLERK_AUTHORIZED_PARTIES`** = `https://www.contractorgo.io,https://contractorgo.io`（无空格，逗号分隔）。**只配 www、用户从根域打开会导致校验失败**；可先不配该项排查，或两域都写上。Edge 代码会在带 parties 失败时**自动再试**不校验 azp，但应在 parties 中补全来源。  
3. **Clerk Dashboard** → **Domains**：已添加 `www.contractorgo.io` / `contractorgo.io`（或你的 Frontend API 域名，如 `clerk.contractorgo.io` 相关配置）。  
4. 修改 Secret 后执行：`supabase functions deploy --project-ref <你的 ref>`，使 `_shared/get-user.ts` 生效。  

更细的逐步排查见 **`docs/DEBUG_AUTH.md`**。

---

## 五、检查清单

- [ ] **用户端**：项目名为 **`contractorgo-web`**，Root Directory **留空**，Framework = **Other**，域名绑 **www.contractorgo.io** / **contractorgo.io**
...
## 七、根域名 contractorgo.io 显示「不安全」、www 正常
