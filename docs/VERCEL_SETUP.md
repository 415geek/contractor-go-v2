...
2. **改错 Vercel 项目**：`www.contractorgo.io` 应绑在**仓库根目录**构建的项目（见上文「用户端 Web App」，`Root Directory` 留空，用根目录 `vercel.json`）。若只在 **`mobile` 子目录**单独建了项目（例如 `c8geeks-projects/mobile`），必须在**该项目**里也配置同一套 `EXPO_PUBLIC_*`，否则预览域名会缺变量。  
